...
public class NXPmParseCSVThread extends NXPmParseThread {
	...
	@Override
	protected void parseXML(File f) throws XMLStreamException, IOException {
		...
		confobj = NXPmParseMain.objcfgs.get(this.objectType);
		if (confobj == null) {
			cr.close();
			return;
		}
		...
		while (cr.readRecord()) {
			 ...
			 * log.debug(pmData.size()); insertObjData(); pmData.clear(); }
			 */
		}
		cr.close();
